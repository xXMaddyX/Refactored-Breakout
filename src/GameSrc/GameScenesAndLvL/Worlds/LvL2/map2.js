...
import { LvL2Music, BackgroundLvL2 } from "../../../CoreSystem/AssetLoader";
import { World2Config, KEYS } from "./map2Config";
...
export default class Map2 {
    ...
    static loadSprites(scene) {
        ...
        scene.load.image("Background2", BackgroundLvL2);
        scene.load.audio("music-lvl2", LvL2Music);
    };
    create() {
        World2Config.backgroundPositions.forEach(({x, y, key, alpha, depth, scale}) => {
            let image = this.scene.add.image(x, y, "Background2");
            image.alpha = alpha;
            image.depth = depth;
            image.scale = scale;
        });

        this.paraBackground = this.scene.add.image(970, 540, "Background2").setScale(2.1).setAlpha(.5).setDepth(-8);
        this.paraBackground2 = this.scene.add.image(950, 540, "Background2").setScale(2.1).setAlpha(.7).setDepth(-9);
